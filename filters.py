...
class CatalogsFilter(FilterSet):
    """adds a filter to the catalogs table"""
    ...
    class Meta:
        model = Catalog
        fields = ("catalog", "sites_search", "sites", "available")
        #'catalog','sites_search', 'sites' ['query', 'title', 'uri', 'comment', 'order', 'locked', 'available', 'sites']
    ...
    def catalog_query(self, queryset, name, search_term, field_name="title"):
        ...
        title_fields = [
            ...
            if i.name.startswith(field_name)
        ]
    ...
    def sites_query(self, queryset, name, search_term):
        """makes a recursive AND query for the sites"""
...
def make_OR_query_fields(fields, value):
    """constructs and combines a list of fields into an OR query"""
    or_Q = reduce(
        lambda x, y: x | y, [Q(**{f"{field}__icontains": value}) for field in fields]
    )
    return or_Q
